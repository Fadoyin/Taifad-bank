...
// register  controller
export const registerController = expressAsyncHandler(async (req: Request<{}, {}, registerType>, res:Response ): Promise<void> => {
    const { 
        email,
        fullName,
      password,
 ...
 await sendMailjetEmail(req, res, option);
   ...
   res.status(201).json({
    status: "success",
    message: "Account verification pending, please verify your email",
    data: registeredUser,
    meta: message
  });
});
...
// login controller
export const loginController = expressAsyncHandler(async (req: Request<{}, {}, {
  email: string,
  password: string
...
// logout controller
export const logOutController = expressAsyncHandler(async (req, res): Promise<void> => {
  res.cookie("token", "", {
    maxAge: 1,
  });
  res.status(200).json({
 ...
 res.status(401).json({
  ...
  const option = {
    ...
    mailData: {
      companyName: "Taifad",
      userName: fullName,
      link:"https://fadoyin-portifolio.s3.eu-north-1.amazonaws.com/portfolio-website-main/index.html",
    }
  };
...
// register user phone
export const registerPhoneController = expressAsyncHandler(async (req: IGetUserAuthInfoRequest, res: Response): Promise<void> => {
...
// verify user phone
export const verifyPhoneController = expressAsyncHandler(async (req:IGetUserAuthInfoRequest, res:Response): Promise<void> => {
  const id = req.user?._id
  const { OTP } = req.body
  if (!id || !OTP) { 
    throw new Error("Missing credentials")
 ...
 await user.save()
  ...
  if (!accountSave) { 
  throw new Error("An error occurred during account creation. Please try verifying your phone number again.")
  }

  res.status(200).json({
    status: "success",
    message: "The account was successfully established and the phone number was correctly validated."
  })
 
});


export const getSingleController = expressAsyncHandler(async (req, res): Promise<void> => {
...
res.status(200).json({
  status: "success",
  mesage: "Users fetched successfuly",
    user: userFound,
  });
});


export const deleteController = expressAsyncHandler(async (req: IGetUserAuthInfoRequest, res:Response): Promise<void> => {
...
await getAccount.save()
 await  TwilloPhoneOtpSender({
  ...
  res.status(200).json({
    status: "success",
    message: "Verification by phone OTP sent; kindly confirm",
    data: getAccount
  })
